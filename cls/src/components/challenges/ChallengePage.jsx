...
function ChallengePage() {
  const { challenge, generateChalenge } = useContext(ChallengContext);
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-8 bg-white rounded-lg shadow-md font-sans text-gray-800">
      <h1 className="text-3xl font-bold text-blue-600 mb-4">{challenge.title}</h1>

      <div className="flex flex-wrap gap-6 mb-6 text-gray-600 text-sm">
        <span>
          <strong>Day:</strong> {challenge.dayNumber}
        </span>
        <span>
          <strong>Skill Level:</strong> {challenge.skillLevel}
        </span>
        <span>
          <strong>Language:</strong> {challenge.preferredLanguage.join(', ')}
        </span>
        <span>
      ...
      <p className="text-lg leading-relaxed mb-8">{challenge.description}</p>
      ...
      {challenge.progress && challenge.progress.length > 0 && (
        <div className="bg-blue-50 p-4 rounded-md text-gray-700">
          ...
          <ul className="list-disc list-inside space-y-2">
            {challenge.progress.map((prog, index) => (
              <li key={index}>
